#ifndef SCMP_LIST_H__
#define SCMP_LIST_H__
//------------------------------------------------------------------------------
#include <atomic>
#include <mutex>
#include <new>
...
template < typename T >
struct SCMPListItem {
  typedef std::atomic< SCMPListItem< T > * > Next;

  Next next;
  T data;
};
//------------------------------------------------------------------------------
template < typename T, typename Allocator = std::allocator< SCMPListItem<T> > >
class SCMPList {
public:
  typedef T ElementType;
public:
  SCMPList() : fRead(&fRoot), fTail(&fRoot) {
    fRoot.next = &fRoot;
  }
  ~SCMPList() {
    Allocator allocator;
...
private:
  typedef SCMPListItem< T > ListItem;
private:
